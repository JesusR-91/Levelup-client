        ...
        <div style={{ padding: "2vh" }}>
          ...
          >
            {publication.map((eachPubl) => (
              ...
              >
                <h5>
                  {eachPubl.owner.username} - <span>{eachPubl.createdAt}</span>
                </h5>
                <p>{eachPubl.content}</p>
                <div>
                  {eachPubl.likes.length > 0 &&
                    (eachPubl.likes.length > 1 ? (
                      <p>{eachPubl.likes.length} Likes</p>
                    ) : (
                      <p>{eachPubl.likes.length} Like</p>
                    ))}
                  {eachPubl.loves.length > 0 &&
                    (eachPubl.loves.length > 1 ? (
                      <p>{eachPubl.loves.length} Likes</p>
                    ) : (
                      <p>{eachPubl.loves.length} Love</p>
                    ))}
                  {eachPubl.dislikes.length > 0 &&
                    (eachPubl.dislikes.length > 1 ? (
                      <p>{eachPubl.dislikes.length} Likes</p>
                    ) : (
                      <p>{eachPubl.dislikes.length} Dislikes</p>
                    ))}
                </div>
                <div
                  style={{
